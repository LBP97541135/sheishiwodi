...
这些素材可以直接用于开发，替换首个里程碑中的几何占位角色。素材接入不得改变已确认的信息层级、漫画分镜尺寸或状态机行为；图片只负责角色表现。
...
| 思考 | [`../../素材/千文-思考.png`](../../素材/千文-思考.png) | 1792 × 2240 | 图像可用；文件名“千文”需改为“千问” |
...
当前名称可以作为原始归档名，不必执行破坏性重命名。接入前端时建议复制并规范化为 `apps/web/src/assets/scenes/interrogation-room.webp`，保留原图。移动端优先使用同一图片配合固定焦点裁切，实际截图验证不足时再补竖版，不提前生成重复素材。
...
接入时建议生成 `apps/web/public/audio/game-bgm.ogg` 与 `game-bgm.mp3` 的压缩副本，原始 WAV 保留为母带。浏览器不得在页面加载时强制自动播放；用户开始游戏或首次主动操作后再播放，并提供静音/音量控制。背景音乐默认循环，切到后台时可以暂停或降低音量；减少动态效果设置不等于自动静音。

## 4. 接入命名

源文件保留在 `素材/` 作为原始归档。接入前端时建议生成规范化副本，不在组件中散落中文路径：
...
每个目录统一使用 `idle.webp`、`thinking.webp`、`speaking.webp`、`suspected.webp`、`eliminated.webp`。角色配置集中维护状态键与文件 URL 的映射，组件不得通过拼接显示名称推导文件路径。

原始 PNG 不删除。规范化副本建议使用统一纵横比和安全裁切区，导出 WebP；具体像素尺寸根据实际分镜容器和响应式截图确定，避免在浏览器中长期加载 1792 × 2240 的大图。
...
1. 立即使用现有 25 张角色图开始开发，接入时生成规范化 WebP 副本。
2. 词牌、投票、印章、气泡、分镜和所有通用 UI 由开发直接编写，不等待额外图片。
3. 通用功能图标和字体在工程初始化时从许可清晰的开源资源中选定并登记。
4. 已有 `背景图.png` 与 `bgm.wav` 可用于原型；接入时分别制作 WebP 场景副本和 Web 压缩音频副本，并提供静音控制。其余装饰物和纸张纹理均为可选，不阻塞功能验收。
