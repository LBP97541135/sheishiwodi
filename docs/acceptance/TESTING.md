...
- 模型调用中断不消耗常规重试预算；继续旧局只恢复中断动作，开始新局产生独立无胜者原因。
- 活动局阻止新复盘启动、在途复盘允许完成、全局复盘并发为 1，重启后恢复持久化队列顺序。
- SQLite 繁忙只重试事务且模型调用次数不增加；完整性异常进入仅健康检查可用的受限诊断模式；迁移前备份可恢复。
...
- 轻量熔断对永久配置错误立即生效，对连续瞬时错误冷却并仅放行一次探测；正常成功调用不受影响。
