...
## 新对局首页信息层级收口（TASK-066）

- [x] 标题“谁”作为可点击的玩家身份入口，同时保留“谁是卧底”的一级标题语义。
- [x] 身份弹层统一编辑名称与人物形象；保存一起生效，取消不落盘。
- [x] 弹层支持自动聚焦、Tab 焦点约束、Esc/遮罩关闭和关闭后焦点返回。
- [x] 页面中央只突出经典模式和猜词模式，经典模式为主操作，猜词模式为较小的未开放入口。
- [x] 难度选择独立放在模式入口下方，不再与姓名或人物形象混排。
- [x] Web 源码测试 52/52、生产构建、1280×720 可见浏览器交互、控制台检查和 `git diff --check` 通过。

## AI 复盘评价提示词精炼（TASK-067）

- [x] 任务先于代码登记，状态设为进行中。
- [x] 单个 AI 评价采用结论先行，并包含最强依据和一条具体改进。
- [x] 关键节点聚焦“行为 → 判断/影响”，总体点评只保留胜负手、关键转折和最大反事实。
- [x] 评分使用统一的 1～5 分锚点，并明确禁止结果论、规则复述、空泛表扬和无事实推断。
- [x] 提示词契约测试、Server typecheck/build、全仓 lint 与 `git diff --check` 通过，完成记录已更新；完整 SQLite 回归的本机 Node ABI 边界已记录。

## GitHub 面试交付说明补强（TASK-068）

- [x] 任务先于文档修改登记，状态设为进行中。
- [x] README 提供可复制的安装、默认假模型启动和真实模型显式启用步骤。
- [x] README 以架构图和表格说明 Multi-Agent harness、信息边界、稳健性与开发者验证责任。
