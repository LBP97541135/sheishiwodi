...
首个里程碑七个切片已经完成：假模型基础玩法、平票、事务/恢复/SSE、放弃、观战、正常终局、事实复盘入口和 E2E 已落地；首版 30 组词库也已补齐。真实模型、格式修复/三次系统重试、`system_terminated`、重复泄词强退、异步 AI 总结、历史列表、Markdown 导出、模型档案、背景渲染与音乐播放器仍未实现。各规格中的后续契约必须明确标注，不能读作当前代码能力。
