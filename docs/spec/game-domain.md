...
- 玩法 `classic | guess`。
- 人类参与模式：一名人类玩家和 3～7 名 AI；纯 Agent 模式：4～8 名 AI。
- 4～5 人局一名卧底，6～8 人局两名卧底。
- 难度 `easy | hard`。
- 人类参与时显示名称为 1 至 12 个字符，并使用两个允许剪影之一。
- Agent 角色 ID 列表必须唯一，且角色符合当前 Provider 的参赛条件。
- 真实 Provider 的纯 Agent 对局必须提供 1～500 的请求预算。

核心领域集合、顺序、概率总量和胜负函数不得依赖固定人数；规则函数读取 `players` 与 `undercoverCount`。
...
首个里程碑曾明确延后异步 AI 复盘、猜词玩法、可选人数、多卧底和阵容选择；这些扩展现已按 DEC-100/101 实现。仍未实现的分支不得以无效按钮或假数据伪装完成。
