...
`CreateGame` 的 `human` 字段使用互斥的两种形态：

```json
{ "displayName": "玩家", "silhouette": "silhouette_a" }
```
...
- 观察者对局仍可携带默认人类字段以保持客户端兼容，但服务端不创建人类席位。
- 使用 `roleId` 时，服务端从权威角色库派生玩家名称与素材，并拒绝不完整、当前 Provider 不可用或同时存在于 `agentRoleIds` 的角色。
- 人类角色选择不会产生 Agent 模型绑定，也不会改变公开事件和命令协议。
- 角色复制使用独立复制命令，返回新建的自建角色；不得把复制请求解释成对来源角色的更新。
