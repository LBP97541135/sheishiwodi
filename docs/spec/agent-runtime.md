...
- 状态：假模型、真实模型运行时、调用台账、结构化上下文审计、出网前边界门禁与轻量熔断已实现；完整调试记录和开发者面板待实现
...
本节对应 DEC-095/096。12.1、12.2 与 12.4 已实现；12.3、完整记录清理器和可选 `TelemetrySink` 仍待开发者面板阶段完成。现有三个模型的超时配置和第 9 节恢复语义不变。
...
`model_attempts` 已通过数据库外键随对局级联删除。结构化清单的按局删除，以及完整 Prompt/响应调试文件的“最多 7 天 + 可配置本地容量上限”、启动清理和主动清除入口，留待服务端可靠性与开发者面板阶段实现；清理器不得删除长期元数据和结构化清单。
...
- 已经发出的请求不因熔断强制取消；熔断错误使用脱敏分类返回，状态投影与变化记录将在开发者面板阶段补齐。
- 熔断器为单进程、单 Provider 的轻量组件；首版本不建设分布式协调。

项目定义的调用语义未来可通过可选 `TelemetrySink` 输出。当前事实源仍为本地 SQLite 与审计文件；第三方平台只能作为未来适配器，不能成为运行依赖或绕过上下文脱敏边界。

## 13. 来源

- 需求：[`../acceptance/REQUIREMENTS.md`](../acceptance/REQUIREMENTS.md) 第 20–29、51–85、109–145、187–192 条。
- 决策：DEC-004、DEC-005、DEC-010 至 DEC-015、DEC-017 至 DEC-021、DEC-029 至 DEC-039、DEC-052 至 DEC-054、DEC-068、DEC-072 至 DEC-074、DEC-082、DEC-086、DEC-092、DEC-093、DEC-095、DEC-096。
