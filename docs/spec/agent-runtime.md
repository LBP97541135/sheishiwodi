...
复盘 Agent 使用独立 `modelId`、提示模板和显式 `gameMode`，但复用所选 Provider 的连接。它只能在终局事实持久化后运行，其输出与确定性事实分开保存和展示。经典模式只评价判断更新、描述/辩解、投票一致性和实际贡献；猜词模式额外生成只评价 AI 的 `guessAnalysis`，包含一段策略总结和最多 3 个关键决策，其中错失机会最多 1 个。人类猜词只作为正常终局事实和环境信息，不生成主观评价。

复盘评价必须结论先行，并按“判断更新、行动一致性、实际影响”的优先级分析。单个 AI 的简评以 60～100 个中文字符为目标，关键节点最多 2 条；总体点评以 100～160 个中文字符为目标。字数属于编辑目标，结构校验仍严格约束玩家完整唯一覆盖、评分、枚举和节点数量，不因轻微字数偏差丢弃合法评价。1～5 分使用统一行为锚点，不按所属阵营最终输赢直接评分。禁止复述规则、身份词牌或完整流程，禁止空泛表扬、编造事实和长段照抄私有信念。

猜词决策使用服务端构造的行动时证据帧。每条新 AI 动作保存 `publicEventCursor`，证据帧只包含 `eventSeq <= publicEventCursor` 的公开事件、该动作的结构化信念和输出；模型只选择允许的 `actionId`，玩家、轮次、阶段、实际猜词/未猜候选由服务端回填。终局结果只能进入 `outcomeImpact`，不得反推 `assessment`；并行投票中共享同一冻结游标的决策不得互相读取返回或结算结果。当前在一次复盘调用内提供这些分区证据，以控制费用；不宣称模型调用级绝对隔离。

猜词专项结构失败后只进行一次针对性修复。若通用逐 AI 评价已合法而专项仍失败，保存通用评价并将 `guessAnalysisStatus` 标记为 `failed`；不得因一个附加区块丢弃整份复盘。
...
- 当前已实现：复盘玩家覆盖、keyMoments 1～2 条、rating 1～5、模式分支和猜词关键节点保持严格结构校验；verdict 60～100、单条关键节点不超过 50、overall 100～160 改为提示词编辑目标。持久化摘要兼容既有较短内容、pending/failed 空内容和猜词专项局部失败。
