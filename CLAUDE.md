...
这是一个 AI 版“谁是卧底”本地全栈项目。产品规则、架构和首个里程碑规格已经确认，首个里程碑七个切片（使用可控假模型的可持久化、可恢复基础玩法）已全部验收。里程碑之外已完成真实模型策略、模型档案、统一 Agent 校验与恢复、角色/场景素材、背景切换、BGM、异步复盘和开发者观测面板；TASK-057 已通过 Web、HTTP、SSE、SQLite、三名真实参赛 Agent 与真实复盘 Agent 的一次性全栈验收，并保留脱敏证据。开发以 `docs/spec/` 为工程契约，以 `docs/tasks/TASKS.md` 和 `docs/tasks/milestone-1-checklist.md` 跟踪实时任务；不要从聊天归档或被替代的旧决策推断当前行为。
...
仓库运行时固定为 `.node-version` 中的 Node 22.14.0，包管理器固定为 `packageManager` 声明的 pnpm 9.15.9；安装原生 SQLite 依赖和执行命令必须使用同一 Node 主版本。
