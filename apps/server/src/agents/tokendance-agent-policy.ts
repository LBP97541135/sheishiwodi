...
function buildMessages(
  ...
  const guessShape = `{${beliefShape}, "action": "guess", "targetPlayerId": "目标玩家ID", "guessedWord": "精确猜测词", "reason": "简短依据"}`;
  const allowedOutputShape = input.guessAvailable
    ? `${outputShape}\n或使用一次性猜词：\n${guessShape}`
    : outputShape;
  ...
  const system =
    ...
    (input.guessAvailable
      ? `本局你尚未猜词。你可以用猜词替代当前行动，但每局仅一次：必须同时选定敌对阵营目标并精确写出其词；成功淘汰目标，任一判断错误则你自己淘汰。投票阶段所有选择基于同一冻结快照并同时结算，不会看到其他人的本批次选择。`
      : '') +
  ...
  const targetsLine =
    input.actionType === 'vote' || input.actionType === 'revote' || input.guessAvailable
      ? `合法目标（只能从中选择）：${input.legalTargets
  ...
  const user =
    ...
    `你的阵营：${input.actor.ownCamp === 'undercover' ? '卧底' : '平民'}。\n` +
...
function repairInstruction(input: AgentTurnInput, error: AgentFormatError): string {
  ...
  const guessContract = input.guessAvailable
    ? `也可猜词，此时顶层必须且只能包含 belief、action、targetPlayerId、guessedWord、reason；action 必须为 guess；targetPlayerId 必须是以下之一：${input.legalTargets.join(', ')}；guessedWord 为 1 至 40 字符。`
    : '';
  return (
    `上一次回复未通过本地校验，脱敏原因：${error.message}。` +
    `请只返回一个 JSON 对象，不要解释、代码块或额外字段。${normalContract}${guessContract}` +
...
function buildOutput(
  ...
  if (input.actionType === 'describe' || input.actionType === 'defend') {
    ...
    if ('action' in result.data) {
      if (!input.guessAvailable || !input.legalTargets.includes(result.data.targetPlayerId)) {
        throw new AgentFormatError('ILLEGAL_TARGET');
      }
      return {
        ...result.data,
        guessedWord: result.data.guessedWord.trim(),
        belief: normalizeBeliefTotal(result.data.belief, livingIds, input.publicConfig.undercoverCount),
      };
    }
    return {
      ...result.data,
  ...
  if ('action' in result.data && !input.guessAvailable) {
    throw new AgentFormatError('ILLEGAL_TARGET');
  }
